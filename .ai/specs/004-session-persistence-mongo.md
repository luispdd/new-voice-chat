...
2. **Collections & Schemas**:
   ...
   - `documents`: `{ _id: string, title: string, content: string, session_id?: string, chunks: [ { chunk_id: string, text: string, embedding: float[], metadata: object } ], created_at: string }`
3. **Indexing**:
   ...
   - Index on `documents.session_id` for fast session-scoped document lookup and cascade deletion.
...
- `DELETE /api/sessions/{session_id}`: Cascading deletion of session, its messages, and attached documents.
- `POST /api/sessions/{session_id}/documents`: Attaches and ingests a `.txt` or `.md` document to the session.
- `GET /api/sessions/{session_id}/documents`: Returns list of documents attached to the session.
